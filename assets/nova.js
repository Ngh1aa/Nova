(() => {
  ...
  function pageHeader(title, description = '', back = '') {
    return `<div class="page-heading"><div>${back ? `<a class="back-link" href="${back}">${icon('back')}<span>Back</span></a>` : ''}<p class="eyebrow">NOVA · Prototype data</p><h1>${title}</h1>${description ? `<p>${description}</p>` : ''}</div><span class="prototype-label">Simulated</span></div>`;
  }
  ...
  function renderKyc() {
    ...
    const stateContent = {
      intro: `<p class="eyebrow">Before capture</p><h2>Prepare one valid photo ID</h2><p>Use good lighting, keep the document flat and allow a few minutes. A production flow would explain why each identity field is required.</p><div class="callout"><strong>Why NOVA asks</strong><p>This prototype represents identity verification but does not submit or verify identity data.</p></div><div class="action-stack"><a class="btn btn-primary" href="app.html?screen=kyc&state=capture">Continue to ID check</a><button class="btn btn-secondary" data-toast="Help and alternate verification are prototype entry points.">I need help</button></div>`,
  ...
  function renderError() {
    const main = `<main id="main" class="main-wrap" tabindex="-1"><section class="empty-state"><div class="empty-mark">${icon('alert')}</div><p class="eyebrow">Transfer not completed</p><h1>Something changed before confirmation</h1><p>NOVA could not revalidate the balance. No money moved. Review the amount and try again.</p><div class="action-stack" style="max-width:460px;margin:20px auto 0"><a class="btn btn-primary" href="app.html?screen=transfer-review">Review again</a><a class="btn btn-secondary" href="app.html?screen=home">Return home</a></div></section></main>`;
    return shell(main);
  }
